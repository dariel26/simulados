...
function getNumeroDeTentativas(idSimulado: string): number {
  const numeroDeTentativas = localStorage.getItem(
    `numeroDeTentativas-${idSimulado}`
  );
  if (numeroDeTentativas === null || numeroDeTentativas === undefined) return 0;
  else return parseInt(numeroDeTentativas);
}
...
function crearTentativa(idSimulado: string): number {
  const numTentativas = getNumeroDeTentativas(idSimulado);
...
function finalizarTentativa(idTentativa: string, idSimulado: string): number {
  ...
  if (tentativaParaFinalizar) {
    ...
    for (const questao of questoes) {
      ...
      if (resposta && resposta.indexSelecionado === questao.alternativaCorreta)
        acertos += 1;
    }
...
const simulados: ISimulado[] = [
  { id: "1", nome: "Prova 1", tentativas: getNumeroDeTentativas("1") },
  { id: "2", nome: "Prova 2", tentativas: getNumeroDeTentativas("2") },
  { id: "3", nome: "Prova 3", tentativas: getNumeroDeTentativas("3") },
  { id: "4", nome: "Prova 4", tentativas: getNumeroDeTentativas("4") },
  { id: "5", nome: "Prova 5", tentativas: getNumeroDeTentativas("5") },
];
